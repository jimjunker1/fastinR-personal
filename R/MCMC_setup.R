...
.spawn <- function(sysfile, nChains, nBurnin, nIter, nThin) {
  ...
  while (!any(grep("MCout", dir()))) {
    Sys.sleep(3)
    cat("+")
  }
  while (length(grep("MCout", dir())) < nChains | sum(file.info(dir()[grep("MCout", dir())])$mtime > orgtime) < nChains) {
    Sys.sleep(3)
    cat("+")
  }
...
.localrun <- function(jagsdata, sysfile, nChains, nBurnin, nIter, nThin) {
  ...
  JM <- sampling(stanmodels[[sysfile]],
    init = function() {
      list(
        props = rep(1 / jagsdata$n_preys, jagsdata$n_preys),
...
diags <- function(MCMCout = NULL, accuracy = 0.01, proba = 0.95, quant = 0.025) {
  ...
  if (rd[[1]]$resmatrix[1] == "Error") {
    ...
    rit <- max(unlist(lapply(rd, function(x) {
      x$resmatrix[, 2]
    })))

    thin <- max(unlist(lapply(rd, function(x) {
      x$resmatrix[, 4]
    })))
...
.delist <- function(datas, Covs = NULL) {
  ...
  if (!is.null(datas$datas.FA)) {
    jagsdata <- c(jagsdata, list(
