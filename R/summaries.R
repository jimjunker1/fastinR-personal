...
summary.pop_props <- function(object, ...) {
  ...
  for (l in 1:object$nChains) {
    ...
    MCMCout <- object[[l]]
    res <- apply(MCMCout, 2, function(x) {
...
summary.ind_props <- function(object, ...) {
  ...
  for (l in 1:object$nChains) {
    ...
    res <- apply(MCMCout[, popix], 2, function(x) {
      quantile(x, c(0.025, 0.5, 0.975))
    })
    ...
    for (a in 1:(length(indix) / n.p)) {
      k <- k + 1
      res <- apply(MCMCout[, indix[seq(k, (n.p - 1) * np + k, np)]], 2, function(x) {
        quantile(x, c(0.025, 0.5, 0.975))
      })
...
summary.cov_props <- function(object, ...) {
  ...
  for (l in 1:object$nChains) {
    ...
    if (nCs > 0) {
      eff <- apply(MCMCout[, betaix[(1:(nCs * (n.preys - 1)) + nGr * (n.preys - 1))]], 2, function(x) {
        quantile(x, c(0.025, 0.5, 0.975))
      })
      k <- 0
      for (i in 1:nCs) {
    ...
    if (nGr > 0) {
      eff <- apply(MCMCout[, betaix[(1:(nGr * (n.preys - 1)))]], 2, function(x) {
        quantile(x, c(0.025, 0.5, 0.975))
      })
      k <- (n.preys - 1)
      for (i in 2:nGr) {
    ...
    for (k in 1:n.preds) {
      res <- apply(MCMCout[, indix[seq(k, (n.preys - 1) * n.preds + k, n.preds)]], 2, function(x) {
        quantile(x, c(0.025, 0.5, 0.975))
      })
