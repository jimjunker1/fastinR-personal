...
simulation <- function() {
  ...
  sim.FA <- function(sep = 1, n.fats = 10, cvar = 0.5) {
    ...
    mprey <- clo(t(apply(preys, c(3), function(x) {
      exp(colMeans(log(x)))
    })))
  ...
  simplott <- function() {
    ...
    plott <- F
    if (all(!is.na(preys))) {
      preya <- cbind(preya, clr(preys))
      preda <- cbind(preda, clr(preds))
      plott <- T
    }
    if (all(!is.na(preys.SI))) {
      preya <- cbind(preya, preys.SI)
      preda <- cbind(preda, t(t(preds.SI) - colMeans(mean_cs)))
      plott <- T
    }
  ...
  output <- gui(simulator,
    ...
    argCommand = list(
      simplot = simplott,
      simProps = guiNestedF(simProps, "Props",
        exec = "Simulate simple proportions",
        argText = list(eveness = "Eveness of simulated diet proportions"),
        argSlider = list(eveness = c(0.05, 1, 0.01)), helps = ""
      ),
      simGroups = guiNestedF(simGroups, "Groups",
        exec = "Simulate grouped diet proportions",
        argText = list(n.groups = "Number of groups"),
        argSlider = list(group.effect.size = c(0, 5, 0.1)), helps = ""
      ),
      simCovs = guiNestedF(simCovs, "Covars",
        exec = "Simulate diet proportions from covariates",
      ...
      FAsim.data = guiNestedF(sim.FA, "FAsim.data",
        ...
        argText = list(
          sep = "Prey separation in FA space",
      ...
      SIsim.data = guiNestedF(sim.SI, "SIsim.data",
        ...
        argText = list(
          sep = "Prey separation in SI space",
    ...
    argText = list(
      FAsim.data = "Simulate Fatty Acid data",
