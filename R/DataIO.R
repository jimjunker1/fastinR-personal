...
add_FA <- function(FA.predators = NULL, FA.preys = NULL, fat.conts = "", Conv.Coeffs.mean = "", Conv.Coeffs.var = "", FC.mean = NULL, FC.var = NULL, CC.mean = 1, CC.var = 1, datas = NULL, LN.par = F) {
  ...
  if (is.character(FA.predators)) {
    ...
    predators <- FA.predators[, -1]
    rownames(predators) <- FA.predators[, 1]
  }
  ...
  if (nchar(fat.conts) == 0) {
    if (length(FC.mean) == n.preys & length(FC.var) == n.preys) {
      ...
      fc.mean <- rep(FC.mean, n.preys)
      fc.var <- rep(FC.var, n.preys)
      fc.data <- 1
    } else {
      print("No fat content provided, fixing to 1")
      fc.data <- 0
      fc.mean <- rep(1, n.preys)
      fc.var <- rep(1, n.preys)
    }
  } else {
