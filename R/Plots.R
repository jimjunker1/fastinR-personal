...
plot.pop_props <- function(x, save = "fastinR_MCMC_", density = T, types = c("cor", "post"), ...) {
  ...
  if (any(types == "post") & density == F) {
    ...
    if (sava == 1) {
      pdf(paste(save, "pop_proportions.pdf", sep = ""))
    } else {
      par(ask = T)
    }
    rpp <- bwplot(variable ~ value,
      data = mp, xlim = c(0, 1), xlab = list(label = expression("Posterior distribution of diet proportions"), cex = 1),
      panel = function(x, y) {
    ...
    if (sava == 1) {
      pdf(paste(save, "_densities.pdf", sep = ""))
    } else {
      par(ask = T)
    }
...
plot.ind_props <- function(x, save = "fastinR_MCMC_", density = T, types = c("cor", "post"), ...) {
  ...
  if (any(types == "post") & density == F) {
    ...
    rpp <- bwplot(variable ~ value,
      data = mp, xlim = c(0, 1), xlab = list(label = expression("Posterior distribution of population diet proportions"), cex = 1),
      panel = function(x, y) {
        grid::grid.segments(1, 0, 0, 0)
        xlist <- split(x, factor(y))
        for (i in seq(along = xlist)) {
  ...
  if (any(types == "post") & density == F) {
    ...
    if (sava == 1) {
      pdf(paste(save, "ind_proportions.pdf", sep = ""))
    } else {
      par(ask = T)
    }
    rpp <- bwplot(variable ~ value,
      data = mp, xlim = c(0, 1), xlab = list(label = expression("Posterior distribution of individual diet proportions"), cex = 1), auto.key = list(
        text = preya.names, points = F,
        rectangles = F,
        lines = FALSE,
...
plot.cov_props <- function(x, save = "fastinR_MCMC_", density = T, types = c("cor", "post"), ...) {
  ...
  k <- 1
  for (n in 1:nGr) {
    colnames(this.eff)[k:(k + n.preys - 1)] <- sprintf(paste("Group", grnames[n], "/ %s"), preya.names)
    k <- k + n.preys
  }
...
multiplot <- function(MCMCouts, save = "fastinR_Multiplot", types = c("violin", "strip", "density")) {
  ...
  if (any(types == "strip")) {
    ...
    if (is.null(names(MCMCouts))) {
      MCnames <- sprintf(paste("MCMC %s"), 1:length(MCMCouts))
    } else {
      MCnames <- names(MCMCouts)
    }

    k <- 1
    for (n in 1:length(MCMCouts)) {
      colnames(outs)[k:(k + n.preys - 1)] <- sprintf(paste(MCnames[n], "/ %s"), preya.names)
      k <- k + n.preys
    }
  ...
  if (any(types == "violin")) {
    ...
    ggs_double_violin <- function(D, num_cat, between_cat, within_cat, x.title, y.title, trans = NULL, breaks = NULL, ylims = NA, scallab = NULL) {
      ...
      if (!is.null(breaks)) {
        m <- ggplot() +
          geom_violin(data = D, aes(x = bet_cat, y = num), col = NA, fill = NA) +
