...
fastinR_GUI <- function() {
  ...
  dispsummaries <- function(Display.Summary = NULL) {
  ...
  saveoutputs <- function(Path = "MCMCout.RData") {
    output <- guiGetSafe("MCMCout")
    save(output, file = Path)
  }

  SaveData <- function(Path = "datas.RData") {
    datas <- guiGetSafe("datas")
    stopifnot(length(datas) > 1)
    save(datas, file = Path)
  }
  LoadData <- function(Path = NULL) {
    load(Path)
    guiSet("datas", datas)
  }
  ...
  output <- gui(.fastin,
    title = "FASTIN main menu",
    argCommand = list(
      add.covs = guiNestedF(add_Covs, "add.covs",
      ...
      SI.data = guiNestedF(add_SI, "SI.data",
        ...
        argText = c(
          SI.predators = "Load predator(s) stable isotope data (csv)",
      ...
      FA.data = guiNestedF(add_FA, "FA.data",
        ...
        argText = c(
          FA.predators = "Load predator(s) fatty acid data (csv)",
    ...
    argText = c(
      SI.data = "Add Stable Isotope data",
