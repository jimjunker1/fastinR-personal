...
quantile.ind_props <- function(object, ...) {
  ...
  for (l in 1:object$nChains) { # for each chain (l)
    ...
    pop.res <- apply(MCMCout[, popix], 2, function(x) {
      quantile(x, c(0.025, 0.5, 0.975))
    })

    # add prey type and chain num
    pop.res <- cbind.data.frame(
      pred = rep(NA, n.prey), prey = prey.names,
    ...
    for (a in 1:(length(indix) / n.prey)) { # for each predator (a)
      k <- k + 1
      ind.res <- apply(MCMCout[, indix[seq(k, (n.p - 1) * np + k, np)]], 2, function(x) {
        quantile(x, c(0.025, 0.5, 0.975))
      })
      ind.res <- cbind.data.frame(
        pred = rep(a, n.prey), prey = prey.names,
...
quantile.pop_props <- function(object, ...) {
  ...
  for (l in 1:object$nChains) {
    MCMCout <- object[[l]]
    chain.res <- apply(MCMCout, 2, function(x) {
      quantile(x, c(0.025, 0.5, 0.975))
    })

    chain.res <- cbind.data.frame(
      prey = prey.names,
